...
class ExpTracker(tf.keras.callbacks.Callback):
    """
    Experiment Tracker saving evaluation results (metric values) in a csv

    :param Path file: root checkpoint store path
    :param String ID: identifier for model run
    :param String description: description of model run
    :param Integer epoch: epoch at which model is evaluated
    :param String factor: name of model type
    :param String exp_type: type of training
    :param String epsilon: setting of epsilon coverage bound parameter
    :param String test_data: evaluated test data subset
    """
    def __init__(self, ID, file, description, epoch, factory, exp_type, epsilon, test_data,    *args, **kwargs):
        super(ExpTracker, self).__init__(*args, **kwargs)
        self.file = file
        ...
        # self.extractor_posterior_temp=extractor_posterior_temp
        # self.gp_posterior_temp= gp_posterior_temp
        # self.spec_norm_bound=spec_norm_bound
        # self.gp_kernel_scale=gp_kernel_scale

    def on_test_end(self, logs=None):
        if logs is not None:
            # compose metrics with percentiles of collecting metrics
            metrics = {}
            for name, value in logs.items():
                metrics[name] = np.mean(value)
                #if isinstance(value, np.ndarray) and np.isnan(value).sum() == 0:
                    #for percentage in PERCENTILES:
                        #metrics[name + f"_{percentage}"]= np.percentile(value, percentage)
            # add additional colums with identifiers
            df = pd.DataFrame([metrics])
            df.insert(0, "ID", self.ID)
            df.insert(1, "test_epoch", self.epoch)
            df.insert(2, "factory", self.factory)
            df.insert(3, "experiment", self.exp_type)
            df.insert(4, "coverage", self.epsilon)
            df.insert(5, "test_data", self.test_data)
            df.insert(6, "description", self.description)
            # df.insert(7, "extractor posterior", self.extractor_posterior_temp)
            # df.insert(8, "gp posterior", self.gp_posterior_temp)
            # df.insert(9, "spec norm", self.spec_norm_bound)
            #df.insert(10, "gp kernel scale", self.gp_kernel_scale)
            # if csv already exists, load it, merge it with new data and save it
            if self.file.exists():
                old_df = pd.read_csv(self.file)
                df = pd.concat([old_df,df], axis=0, join="outer") # missing values are filled with NaNs
            # save data to csv
            df.to_csv(self.file, sep=",", float_format="%.4f", index=False, header=True, mode="w")
...
class TFSaver(tf.keras.callbacks.Callback):
    ...
    def __init__(
        self, path, model, optimizer, measures=['epoch'], is_min=[False], last_n=5, start_delay=0
    ):
        ...
        self.last_n = last_n
        if start_delay==0 and hasattr(model, 'loss_scaler'):
            self.start_delay = 30
        else:
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        resolved_path = None
        if epoch > self.start_delay: # start delay might be necessary in variational runs (especially with limited data availability)
...
class CollectingMetricWrapper(Metric):
    ...
    def result(self):
        return self.values[:self.used_values]#tf.reduce_mean(self.values[: self.used_values])
...
class MultivariateNormalDiag(DistributionLambda):
    ...
    def new(params, event_shape=(), validate_args=False, name=None):
        ...
        with tf.name_scope(name or 'MultivariateNormalDiag'):
            ...
            # bias in scale_diag avoids NaN during optimization
            # factors in loc and scale_diag reparameterize optimization, avoiding to steep gradients
            # see https://github.com/tensorflow/probability/issues/703
            return mvn_diag_lib.MultivariateNormalDiag(
                loc=tf.cast(1e-1*tf.reshape(loc_params, output_shape), tf.float32),
                # scale_diag= tf.ones(output_shape),
                scale_diag = tf.cast(1e-3 + tf.math.softplus(1e-2 * tf.reshape(scale_params, output_shape)), tf.float32),
