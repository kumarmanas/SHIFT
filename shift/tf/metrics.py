...
Joining = namedtuple("Joining", ["metrics", "outputs"])
...
def displacement_error(y_true, y_pred, final_only=False, k=1, mapper=None):
    """
    Determines the minimal average and final displacement error (ADE/FDE) of the top k predictions. Projects predictions and/or groundtruth into a coordinate space, if required.
...
def stop_line_compliance(y_true, y_pred, mapper=None):
    """
    Determines the drivable area compliance by counting all most likely predictions that are drivable. Projects 
...
# def brier_score(y_true, y_pred, mapper):
#     """
#     Determines the average and final brier-displacement error (ADE/FDE) weighted. Projects predictions and/or groundtruth into categorical space, if required.
...
#     if mapper:
#         y_true, y_pred = mapper(y_true, y_pred)
#     result = (1-tf.gather(y_pred, y_true, axis=-1, batch_dims=1))**2+tf.reduce_sum(y_pred**2, axis=-1)-tf.gather(y_pred, y_true, axis=-1, batch_dims=1)**2
#     return result

# def relative_displacement_error(y_true, y_pred, final_only=False, k=1, mapper=None):
#     """
#     Determines the relative average and final displacement error (ADE/FDE) of the top k trajectories. Projects predictions and/or groundtruth into a coordinate space, if required.
...
#     ade = tf.norm(y_true - y_pred, axis=-1)
#     ade = tf.reduce_mean(ade, -1)
#     result = tf.reduce_mean(ade, -1) - tf.reduce_min(ade, -1)
#     return result
